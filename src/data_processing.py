...
class StockData(BaseModel):
    ...
    def load_transform(self) -> pd.DataFrame:
        ...
        """
        logging.info(
            f"Initializing dataset for ${self.ticker} stock from {self.start_date} to {self.end_date}."
        )

        # Download stock data
        stock_data = yf.download(
            self.ticker, start=self.start_date, end=self.end_date, auto_adjust=False
        )
        if stock_data.empty:
            logging.error(
                f"No data found for {self.ticker} from {self.start_date} to {self.end_date}."
            )
    ...
    def partition_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logging.info(
            f"Train split has {train.Extreme_Event.value_counts()[0]} normal events and {train.Extreme_Event.value_counts()[1]} extreme events."
        )
        logging.info(
            f"Validation split has {val.Extreme_Event.value_counts()[0]} normal events and {val.Extreme_Event.value_counts()[1]} extreme events."
        )
        logging.info(
            f"Test split has {test.Extreme_Event.value_counts()[0]} normal events and {test.Extreme_Event.value_counts()[1]} extreme events."
        )
    ...
    @staticmethod
    def get_windows(
        df: pd.DataFrame, lookback: int, horizon: int, target: str
